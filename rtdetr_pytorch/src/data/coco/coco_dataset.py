...
# ===== for custom COCO datasets (PlantDoc etc.) =====
...
class CocoDetection(torchvision.datasets.CocoDetection):
    ...
    def __init__(self, img_folder, ann_file, transforms, return_masks, remap_mscoco_category=False):
        ...
        global mscoco_category2label, mscoco_label2category
        mscoco_category2label, mscoco_label2category = build_mscoco_category_maps(self.coco)
...
# === FIX for custom COCO-style datasets (e.g. PlantDoc) ===
# === FIX for custom COCO-style datasets (e.g. PlantDoc) ===
def build_mscoco_category_maps(coco):
    cat_ids = [cat['id'] for cat in coco.dataset['categories']]
    cat_ids = sorted(cat_ids)
    cat2label = {cat_id: i for i, cat_id in enumerate(cat_ids)}
    label2cat = {v: k for k, v in cat2label.items()}
    return cat2label, label2cat
